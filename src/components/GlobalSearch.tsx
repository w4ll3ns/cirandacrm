...
export default function GlobalSearch() {
  ...
  const results = useMemo(() => {
    ...
    responsaveis.filter(r => r.nome.toLowerCase().includes(q) || (r.whatsapp || r.telefone || '').includes(q)).slice(0, 5).forEach(r => {
      items.push({ type: 'Responsável', label: r.nome, sub: r.whatsapp || r.telefone, path: `/app/contatos/resp/${r.id}`, icon: Users });
    });

    alunos.filter(a => a.nome.toLowerCase().includes(q)).slice(0, 5).forEach(a => {
      items.push({ type: 'Aluno', label: a.nome, sub: a.serie_interesse || '-', path: `/app/contatos/aluno/${a.id}`, icon: GraduationCap });
    });
  ...
  return (
    <div ref={ref} className="relative w-72">
      ...
      <input value={query} onChange={e => setQuery(e.target.value)} onFocus={() => setFocused(true)} placeholder="Buscar contatos, oportunidades..." className="w-full pl-10 pr-8 py-2 bg-muted rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
      {query && (
      ...
      {showDropdown && (
        <div className="absolute top-full left-0 right-0 mt-1 bg-card border border-border rounded-xl shadow-lg overflow-hidden z-50">
          {results.map((r, i) => (
            <button key={i} onClick={() => { navigate(r.path); setQuery(''); setFocused(false); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-left hover:bg-muted transition-colors text-sm">
