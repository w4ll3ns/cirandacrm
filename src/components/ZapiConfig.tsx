...
import { Wifi, WifiOff, Save, Trash2 } from 'lucide-react';
...
export default function ZapiConfig() {
  ...
  const handleSave = async () => {
    if (!form.nome_instancia || !form.instance_id || !form.token) {
  ...
  return (
    <div className="space-y-3">
      ...
      {!showForm ? (
        <button onClick={() => setShowForm(true)} className="text-xs bg-primary/10 text-primary px-3 py-1.5 rounded-full font-medium">
          + Nova Instância
        </button>
      ) : (
        <div className="space-y-2 p-3 bg-muted/30 rounded-lg border border-border">
          ...
          <input value={form.client_token} onChange={e => setForm({ ...form, client_token: e.target.value })} placeholder="Client Token (opcional)" className="w-full bg-background rounded-lg px-3 py-2 text-sm border border-border focus:outline-none focus:ring-2 focus:ring-primary" />
          <div className="flex gap-2">
            <button onClick={() => setShowForm(false)} className="flex-1 py-2 text-sm text-muted-foreground font-medium">Cancelar</button>
