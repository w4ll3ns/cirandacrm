...
import { ArrowLeft, MessageCircle, Calendar, CheckCircle2, XCircle, ChevronRight } from 'lucide-react';
    ...
    <div className="min-h-screen bg-muted">
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          <button
            ...
            className="bg-success text-success-foreground rounded-xl py-3 text-sm font-semibold flex items-center justify-center gap-2 active:scale-95 transition-transform"
          >
            <MessageCircle className="w-4 h-4" /> WhatsApp
          </button>
          <button
            onClick={() => setShowMoveSheet(true)}
            className="bg-secondary text-secondary-foreground rounded-xl py-3 text-sm font-semibold flex items-center justify-center gap-2 active:scale-95 transition-transform"
          >
      ...
      {showMoveSheet && (
        <div className="fixed inset-0 z-50 flex items-end" onClick={() => setShowMoveSheet(false)}>
          ...
          <div className="relative w-full bg-card rounded-t-2xl p-4 max-h-[60vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              ...
              {ETAPAS_ORDER.map(etapa => (
                <button
                  ...
                  className={`w-full text-left px-4 py-3 rounded-lg text-sm font-medium transition-colors ${
                    etapa === opp.etapa ? 'bg-primary/10 text-primary' : 'active:bg-muted'
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setShowLostDialog(false)}>
          ...
          <div className="relative bg-card rounded-2xl p-5 w-full max-w-sm" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-2 mt-3">
              <button onClick={() => setShowLostDialog(false)} className="flex-1 py-2 text-sm font-medium text-muted-foreground">
                Cancelar
              </button>
              <button onClick={handleLost} className="flex-1 py-2 bg-destructive text-destructive-foreground rounded-lg text-sm font-semibold">
                Confirmar
              </button>
            </div>
          </div>
