      ...
      <div className="min-h-screen bg-muted">
        <div className="bg-primary text-primary-foreground px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-semibold truncate">{resp.nome}</p>
            <p className="text-xs opacity-80">Responsável</p>
          </div>
        </div>
        <div className="p-4 space-y-4">
          <div className="bg-card rounded-xl p-4 border border-border space-y-2">
            ...
            {resp.tags.length > 0 && <div className="flex gap-1">{resp.tags.map(t => <span key={t} className="bg-accent/20 text-accent-foreground text-[10px] px-2 py-0.5 rounded-full">{t}</span>)}</div>}
          </div>

          <button onClick={() => toast.success('Abrindo WhatsApp...')} className="w-full bg-success text-success-foreground rounded-xl py-3 text-sm font-semibold flex items-center justify-center gap-2 active:scale-95 transition-transform">
          ...
          {relAlunos.length > 0 && (
            <div className="bg-card rounded-xl p-4 border border-border">
              ...
              {relAlunos.map(a => (
                <button key={a.id} onClick={() => navigate(`/app/contatos/aluno/${a.id}`)} className="w-full text-left py-2 text-sm flex items-center gap-2">
          ...
          {relOpps.length > 0 && (
            <div className="bg-card rounded-xl p-4 border border-border">
              ...
              {relOpps.map(o => (
                <button key={o.id} onClick={() => navigate(`/app/oportunidades/${o.id}`)} className="w-full text-left py-2 text-sm flex items-center justify-between">
          ...
          {relConvs.length > 0 && (
            <div className="bg-card rounded-xl p-4 border border-border">
              ...
              {relConvs.map(c => (
                <button key={c.id} onClick={() => navigate(`/app/conversas/${c.id}`)} className="w-full text-left py-2 text-sm flex items-center gap-2">
    ...
    <div className="min-h-screen bg-muted">
      <div className="bg-primary text-primary-foreground px-4 py-3 flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs opacity-80">Aluno · {aluno.serie_turma_interesse}</p>
        </div>
      </div>
      <div className="p-4 space-y-4">
        <div className="bg-card rounded-xl p-4 border border-border space-y-2">
          ...
          {resp && (
            <button onClick={() => navigate(`/app/contatos/resp/${resp.id}`)} className="text-sm text-primary font-medium">
